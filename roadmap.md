...
- 📖 **v92 第 10 本譯本:和修本(和合本2010, `rcuv`)＝線上譯本**(使用者點名「想多增加一個譯本:和修本」)。
  信望愛 `qb.php?version=rcuv` 逐章即時查。**刻意不打包**:`abv.php` 對 rcuv 標 `candownload=0`
  (不可下載離線資料庫),`/json/` 文件明載「有些譯本僅授權給信望愛站使用…請勿任意使用,以免違法」
  ⇒ 查詢導流而非重新散布。三件呈現(使用者拍板「B 路 + 註腳可點開 + 標題外提」):
  ①段落標題外提到該節之上(節文內嵌 `<h2>` 留在句子裡會爆大標題)②譯註抽成上標 `[n]` 鈕點開展開
  ③**純淨經文進 `bibleData.rcuv`、標題註腳進 `rcuvMeta` 側車** ⇒ 複製/朗讀/字級對比自動拿到乾淨版
  (結構保證,非靠呼叫端記得)。清洗規則**寫進頁面前先對真實資料驗過** 16 章 619 節,零殘留。
  全 66 卷可用(`chineses=` 逐卷實測;`engs=John` 會回羅馬書,別用)。
- 🩹 **v93 修 v92 隱形地雷**:註腳佔位符是**字面 NUL 位元組**而非 `'\0'` 轉義,2 個 U+0000 被烤進
  `index.html` 並已部署(功能正常、線上驗過,但原始碼裡看不見)。★ 抓到它的是把同一份程式抄進
  7bible 後 `git diff` 顯示 `- -`(binary)——**git binary 判定只掃前 8000 位元組**,index.html 的
  NUL 在 offset ~133,700 躲過了,4KB 的 `rcuv-core.js` 藏不住。**大檔比小檔更容易藏住同一個病。**
- 🐛 同輪修掉的三個真 bug(都是驗收才抓到,不是寫的時候想到的):
  ①**並排欄寬被擠窄**:註腳框當 grid 第 3 個項目 ⇒ `auto-fit` 空軌不再收合,兩譯本 453→226px
  右側留白;`grid-column:1/-1` 只解決跨欄 ⇒ 改掛在 grid 外面。**截圖才看得出來。**
  ②**斷網印出騙人的話**:和修本仍排 47 列寫「此版本無此節經文」(經文存在,是沒取到)。
  真因=第一次修錯地方:`localVers` 只管收資料,**列是從 `vers` 排的** ⇒ `renderVerseGroup` 要收
  章層過濾後的清單。現在斷網=0 列+失敗橫幅+重試鈕。
  ③**我的驗證在讀隱藏 DOM**:分頁停在 quick 時 `#panel-read` 是 `display:none`,而 `innerText`
  對未渲染元素**退回 textContent** ⇒「驗過了」其實沒上螢幕(且第一次讀到的是約 1 不是約 3)。
  改量 `getBoundingClientRect` 才算真的可見。
- 📋 **待使用者拍板(別擅自動工)**:`abv.php` 顯示 `ncv`(新譯本)與 `esv` 也是 `candownload=0`,
  而這兩本目前仍打包在 `data/`。要不要改成線上、或維持現狀,是使用者的決定。

---

## ✅ 已完成(2026-08-10,DEYI 機——v88~v91 已部署上線)

- 🔤 **v91 彈窗頂固定「本節原文」**(使用者:「7bible 每節都有原文,8bible 註釋為何不能?」——答案=
  可以,換資料源):註釋本文的編號受限 FHL 資料現實(很多卷沒放),但 **qb.php 帶標和合本全 66 卷
  每節都有** ⇒ 兩源並用:彈窗頂固定一塊「🔤 本節原文(和合本,點詞就地看字義)」,點詞(可含多編號
  +文法碼)插中文字義卡;上一段/下一段會跟著換節;fetchStrongsVerse=v88 貼進來的 strongs core
  本來就有,正好用上。帖前 1:1(註釋零編號卷)實測 19 詞可點、「保羅」→SG 3972。
  ⇒ **v88 立的「本站刻意不做點經文詞」界線更新**:主閱讀畫面仍不做;註釋彈窗內=使用者點名要,已做。)
...
| 7 | **字級對比(DIFF_VERSIONS)接和修本** | 功能 | ★★★ | ~1hr | 0814 明確未做:`DIFF_VERSIONS` 是另一份清單;和修本 vs 和合本的用字差異正是對讀最有價值的一組。需先確保該章已抓回(比照 `hasChapterData`) |
| 8 | **和修本快取上限與清理 UI** | 功能 | ★★ | ~1hr | 現為 `bible-rcuv-cache-v1` 上限 60 章、LRU 汰換、刻意不進備份鏈;若使用者回報「離線想看之前讀過的和修本」再考慮加「釘選這章離線保留」 |
| 待議 | 串珠交叉引用移植（0809 Opus 5 二審主張退貨/我方保留） | 功能 | — | ~1天 | **若做必須預設收合在進階**，簡易模式已落地（v85），可再議＝大表「待議」列 |
| 待拍板 | **`ncv`/`esv` 是否比照和修本改成線上** | 決策 | — | — | `abv.php` 標二者 `candownload=0` 而目前打包在 `data/`。**使用者決定,別擅自改**——改成線上會失去離線能力,是取捨不是修 bug |

> 0814 完工移出：和修本線上譯本（v92）、NUL 位元組修（v93）——見上方已完成段。
...
- **把和修本打包成 `data/rcuv.json`** — 信望愛 `abv.php` 對 rcuv 標 `candownload=0`（不可下載離線
  資料庫），文件並註明部分譯本僅授權該站使用。要離線就得重新散布，那是版權問題不是技術問題。
  ⇒ 永久走線上查詢。（同理：不要寫爬 1,189 章的腳本去抓它。）
- **和修本納入全文搜尋** — 手上只有讀過的章、不是全本；就算拿命中清單逐節補查，一次搜尋可能要向
  信望愛抓上百章＝把志工營運的服務當自己的資料庫用。改成在畫面明示「搜尋不含線上譯本」。
- **`version=unv,rcuv` 逗號串多譯本省呼叫次數** — 實測會回**整本聖經 31,103 節 / 5.8MB**，
  而 `status` 仍是 `success`。一個版本一次呼叫。
