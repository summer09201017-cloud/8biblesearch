...
const CACHE_NAME = 'bible-multi-v96';   /* v96(0816):併節記號修——使用者截圖回報「查詢結果出現 a」(撒上25:31 和合本/呂振中)。真相:那是信望愛資料庫的「併節」記號(內容在上一節),中文三譯本共 144 節,API 上游即如此、重抓無用。修法照 FHL 自家閱讀器慣例顯示【併於上節】,在 index.html 載入點一次正規化,data/*.json 不動(已快取的離線資料不必重抓)。 v95(0814):🎙 即時朗讀改神經人聲(曉臻)——使用者拍板 A 案。不再用 Web Speech(那就是他在 iPhone 聽到的機器聲);_speakNext/_pickVoice 已移除,無退路(守門 #27)。走 hfpc-tts Worker,伺服器 SSML 真調速,邊播邊抓下一段;拿不到音檔明說「需要連線」。同修 hfpc-tts 的 Edge 版本字串過期(見該 repo)。 v94(0814):🔊 朗讀唸標點修(使用者回報 iPhone「連標點符號一起朗讀」)——
